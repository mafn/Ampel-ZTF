#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : ampel/ztf/t0/DelayedT0Controller.py
...
class DelayedT0Controller:
	...
	def run_alertprocessor(pos, radius, dt, channels, raise_exc=False):
		...
		"""
		archive = ArchiveDB(AmpelConfig.get('resource.archive.reader'))
...
def listen(opts):
	...
	sources = []
	for resource in pkg_resources.iter_entry_points('ampel_target_sources'):
		klass = resource.resolve()
		base_config = {k: AmpelConfig.get('resource.{}'.format(k)) for k in klass.resources}
		run_config = {}
