...
# Conflicts with 0.8.3 in Ampel-plot
...
def get_fpbot_baseline(df: pd.DataFrame,
		window = "10D", min_peak_snr = 3, risetime = 100, falltime = 365) -> pd.DataFrame:
	"""
	For each unique baseline combination, estimate and store baseline.
	Partially taken from https://github.com/BrightTransientSurvey/ztf_forced_phot/blob/main/bts_phot/calibrate_fps.py.
...
class ZTFFPbotForcedPhotometryAlertSupplier(BaseAlertSupplier):
	...
	def __next__(self) -> AmpelAlertProtocol:
		...
		# Look for coordinates from header
		ra, dec, lines, snname = None, None, [], None
		for bl in fileio.readlines():
			m = re.search("#(ra|dec|name)=(.+)", str(bl, 'UTF-8'))
			if m:
				if m.group(1)=='ra':
					ra = m.group(2)
				elif m.group(1)=='dec':
		...
		if self.excl_poor_conditions:
			df = df[ (df['ampl.err']>0) & (df['chi2dof']<3) & (df['cloudy']==0) & (df['infobits']==0) ]
		...
		if self.plot_suffix and self.plot_dir:
			...
			plt.tight_layout()
			plt.savefig( os.path.join(self.plot_dir, 'fpbase_%s.%s'%(snname, self.plot_suffix)) )
			plt.close('fig')
			plt.close('all')
			del(fig)
			gc.collect()
...
#				continue
		...
		return AmpelAlert(
			...
			stock = to_ampel_id(snname), # internal ampel id
